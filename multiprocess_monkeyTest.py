...
# 多进程启动
class MonkeyProcess(Process):
    def __init__(self, dev, package_list):
    ...
    def run(self):
        ...
        start(self.dev, package_name)


def create_process_monkey(package_list):
    process_instances = []
    if device_list != []:
        for id_device in device_list:
            dev = id_device
            instance = MonkeyProcess(dev, package_list)
...
if __name__ == '__main__':
    ...
    if ba.attached_devices():
        create_process_monkey(package_list)
    else:
        print("设备不存在")
